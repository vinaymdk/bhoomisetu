...
export const SellerInterestsPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        const resp = await mediationService.getPropertyInterests({
  ...
  return (
    <div className="mediation-page">
      <div className="mediation-header">
        <div>
          <h1>Interests in My Properties</h1>
          <p>Track buyer interest and CS mediation for your listings.</p>
        </div>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="mediation-state empty">No interests yet. We will notify you when buyers express interest.</div>
      )}
